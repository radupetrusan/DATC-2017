...
namespace AlbumPhoto.Service
{
    public class AlbumFotoService
    {
        ...
        public static string GetBlobUrl(string fileName)
        {
            ...
            CloudBlobClient sasBlobClient = new CloudBlobClient(storageAccount.BlobEndpoint, new Microsoft.WindowsAzure.Storage.Auth.StorageCredentials(sas));
        ...
        public Link GetLink(string poza)
        {
            ...
            foreach (var file in query)
            {
                ...
                {
                    link.linkPoza = GetBlobUrl(file.Url);
                }
                link.poza = poza;
            }
            return link;
        ...
        public List<Comment> GetComments(string poza)
        {
            ...
            foreach (var file in query)
            {
                if (file.Text != null)
                {
                    string[] v = file.Text.Split(new string[] { "#%#" }, StringSplitOptions.None);
        ...
        public void UploadComment(string userName, string comment, string writtenBy, Stream continut)
        {
            var blob = _photoContainer.GetBlockBlobReference(writtenBy);
            blob.UploadFromStream(continut);
            _ctx.AddObject(_commentsTable.Name, new CommentEntity(userName, writtenBy)
            {
                Text = comment,
                MadeBy = writtenBy
            });
