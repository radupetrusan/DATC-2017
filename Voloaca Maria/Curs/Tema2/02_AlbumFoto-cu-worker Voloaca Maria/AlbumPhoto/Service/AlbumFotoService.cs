...
namespace AlbumPhoto.Service
{
	public class AlbumFotoService
	{
        ...
        public void IncarcaComm(string v, string photo, string comm)
        {
            _ctx.AddObject(_commentsTable.Name, new CommentEntity(v, photo)
            {
                MadeBy = v,
                Text = comm
            });
        ...
        public List<Poza> GetPoze()
		{
			...
			{
                var comment = new List<Comm>();
                foreach(Comm c in comm )
                {
                    if(c.Description==file.RowKey)
                    {
                        comment.Add(c);
                    }
                }
        ...
        public Poza GenerareLink(string poza)
        {
            ...
            var poze = new List<Poza>();
            var query = (from file in _ctx.CreateQuery<FileEntity>(_filesTable.Name)select file).AsTableServiceQuery(_ctx);
